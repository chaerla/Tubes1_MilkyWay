...
public class BotService extends CheckEffect{
    ...
    public void computeNextPlayerAction(PlayerAction playerAction) {
        ...
        if (!players.isEmpty()) {
            ...
            if (!strategied && degreeValid && !isUsingShield(opponents.get(0).effects) && bot.hasTorpedo() && (opponents.get(0).getSize() < bot.getSize() + bot.torpedoSalvoCount*10) && (distanceToOpp < 50)) {
            ...
            if (torpedoList.size() > 0){
                ...
                if (bot.hasShield() && !isUsingShield(bot.effects) && (bot.getSize() > 30) && (getDistanceBetween(bot, torpedoList.get(0)) < bot.getSize() + 50) && heading_gap(headingToOpp, torpedoHeading) > 5) {
